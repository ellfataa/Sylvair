...
def main():
    ...
    # A. Drop kolom indeks yang tidak relevan
    if 'No' in df.columns:
        df = df.drop(columns=['No'])
        
    # B. Handling Missing Values (Modus Imputation)
    print("-> Mengecek dan menangani missing values...")
    imputer = SimpleImputer(strategy='most_frequent')
    df_imputed = pd.DataFrame(imputer.fit_transform(df), columns=df.columns)
        
    # C. Label Encoding (Mengubah kategori teks menjadi numerik)
    encoders = {}
    ...
    # D. Pemisahan Fitur (X) dan Target (y)
    ...
    # 5. EVALUASI DAN KOMPARASI MODEL terbaik
    ...
    # Menyeleksi model dengan performa akurasi tertinggi
    if xgb_acc > rf_acc:
